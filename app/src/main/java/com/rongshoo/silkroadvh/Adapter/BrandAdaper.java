 ...
 */
public class BrandAdaper extends BaseQuickAdapter<BrandBean.DataBean.RecordBean, BaseViewHolder> {

    //alt+insert constructor 快速构造方法
    public BrandAdaper(int layoutResId, List<BrandBean.DataBean.RecordBean> data) {
        super(layoutResId, data);
    }

    // 第二种
    public BrandAdaper(List<BrandBean.DataBean.RecordBean> data) {
        super(R.layout.item_brand, data);
    }

    @Override
    protected void convert(BaseViewHolder baseViewHolder, BrandBean.DataBean.RecordBean map) {
        //baseViewHolder.setIsRecyclable(false);//关闭复用
        CheckBox box = baseViewHolder.getView(R.id.name_tv);
        box.setText(map.getSbrandname());
        if (map.getChecked()) {
            box.setChecked(true);
        } else {
